    ...
    def add_slime_arguments(parser):
        ...
        def add_on_policy_distillation_arguments(parser):
            ...
            parser.add_argument(
                ...
                default=False,
                help=(
                    "Enable OPD-SGLang hybrid training with an additional online SFT sample generated by "
                    "the external teacher for each student rollout prompt."
                ),
            )
            parser.add_argument(
                ...
                type=float,
                default=1.0,
                help="Loss coefficient for teacher-generated online SFT samples. Default is 1.0.",
            )
            parser.add_argument(
                ...
                default=None,
                help=(
                    "Maximum new tokens for teacher-generated SFT samples. "
                    "Defaults to --rollout-max-response-len when unset."
                ),
            )
            parser.add_argument(
                ...
                type=float,
                default=0.0,
                help="Sampling temperature for teacher-generated SFT samples. Default is 0.0.",
            )
            parser.add_argument(
                "--opd-teacher-sft-top-p",
                type=float,
                default=1.0,
                help="Top-p for teacher-generated SFT samples. Default is 1.0.",
            )
            parser.add_argument(
                "--opd-teacher-load",
                type=str,
...
def slime_validate_args(args):
    ...
    if (
        getattr(args, "opd_teacher_sft_max_response_len", None) is not None
        and getattr(args, "opd_teacher_sft_max_response_len") <= 0
    ):
        raise ValueError("--opd-teacher-sft-max-response-len must be a positive integer when set.")
    if getattr(args, "opd_teacher_sft_temperature", 0.0) < 0:
        raise ValueError("--opd-teacher-sft-temperature must be >= 0.")
    if not (0 < getattr(args, "opd_teacher_sft_top_p", 1.0) <= 1.0):
        raise ValueError("--opd-teacher-sft-top-p must be in (0, 1].")
    ...
    if args.use_opd:
        ...
        if args.opd_type == "megatron":
            ...
            if getattr(args, "opd_kl_mode", "token_reverse_kl") in (
                "full_vocab_topk_reverse_kl",
                "topk_reverse_kl_notail",
                "topk_reverse_kl_notail_sg",
            ):
            ...
            if getattr(args, "opd_teacher_sft", False):
                raise ValueError("--opd-teacher-sft currently only supports --opd-type=sglang.")
            if getattr(args, "opd_kl_mode", "token_reverse_kl") != "token_reverse_kl":
                raise ValueError("--opd-kl-mode only supports --opd-type=sglang.")
            if args.opd_teacher_load is not None:
                raise ValueError(
        ...
        if getattr(args, "opd_teacher_sft", False):
            raise ValueError("--opd-teacher-sft is set but --use-opd is not enabled.")
        if getattr(args, "opd_token_stats", False):
    ...
    if getattr(args, "opd_teacher_sft", False) and getattr(args, "opd_type", None) != "sglang":
        raise ValueError("--opd-teacher-sft currently only supports --opd-type=sglang.")
